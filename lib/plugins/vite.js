...
export function xrayReactVitePlugin(params = {}) {
  ...
  if (runServer) {
    try {
      ...
      const totalUsageFiles = Object.keys(usageMap).length;
      const totalUsageComponents = Object.values(usageMap).reduce((sum, components) => sum + components.length, 0);
      ...
      const totalImportFiles = Object.keys(importMap).length;
      const totalImportComponents = Object.values(importMap).reduce((sum, components) => sum + components.length, 0);
  ...
  return {
    ...
    transformIndexHtml(html, context) {
      ...
      if (isServerBuild) {
        console.log('xray-react: Skipping SSR build');
        return html;
      }
