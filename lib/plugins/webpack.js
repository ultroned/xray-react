        ...
        (assets, callback) => {
          try {
            ...
            if (asset) {
              ...
              console.log(`xray-react: Injected scripts into ${assetName}`);
            } else {
      ...
      compiler.hooks.done.tap(pluginName, (stats) => {
        if (!this.serverIO) {
          try {
            ...
            const totalUsageFiles = Object.keys(this.usageMap).length;
            const totalUsageComponents = Object.values(this.usageMap).reduce((sum, components) => sum + components.length, 0);
            console.log(`xray-react: Built usage map: ${totalUsageFiles} files, ${totalUsageComponents} component usages`);
            
            this.importMap = buildImportMap(sourcePaths);
            const totalImportFiles = Object.keys(this.importMap).length;
            const totalImportComponents = Object.values(this.importMap).reduce((sum, components) => sum + components.length, 0);
            console.log(`xray-react: Built import map: ${totalImportFiles} files, ${totalImportComponents} component imports`);
            ...
            this.serverIO.on('connection', (socket) => {
              ...
              console.log(`xray-react: Sent project root to client: ${projectRoot}`);
              
              socket.emit('usage-map', { usage: this.usageMap });
              console.log(`xray-react: Sent usage map to client: ${Object.keys(this.usageMap).length} files`);
              
              socket.emit('import-map', { imports: this.importMap });
              console.log(`xray-react: Sent import map to client: ${Object.keys(this.importMap).length} files`);
