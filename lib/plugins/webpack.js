...
export class XrayReactWebpackPlugin {
  constructor(params = {}) {
    ...
    this.mode = AVAILABLE_UI_MODES.includes(params.mode) ? params.mode : UI_MODE_FULL;
    this.sources = {};
    this.usageMap = {};
    this.importMap = {};
    this.serverIO = null;
  ...
  getOutputFileName(options = {}) {
    return this.params.output || options?.output?.filename || null || 'bundle.js';
  }
  ...
  checkModuleResource(resource, sourcePath) {
    ...
    const isNeededSource =
      resource.includes(sourcePath) &&
      ...
      compilation.hooks.processAssets.tapAsync(
        ...
        (assets, callback) => {
          try {
            ...
            const pathToUIFile = path.resolve(
              dirname,
              'node_modules/xray-react/build/xray-react-ui.min.js',
            );
            const pathToClientFile = path.resolve(
              dirname,
              'node_modules/xray-react/build/xray-react-client.min.js',
            );
            ...
            if (!asset) {
              const mainChunkNames = ['main', 'pages/_app', 'pages/_app-client', 'framework'];
            ...
            if (asset) {
              ...
              if (typeof asset.source === 'function') {
                ...
                currentSource =
                  typeof asset._source.source === 'function'
              ...
              combinedSource += `\nwindow.__XRAY_REACT_MODE__='${this.mode}';`;
              combinedSource += '\n' + uiScript;
              ...
              console.warn(
                `xray-react: Could not find suitable asset to inject scripts. Available assets: ${Object.keys(assets).slice(0, 10).join(', ')}...`,
              );
            }
          ...
          for (const module of modules) {
            ...
            if (this.checkModuleResource(resource, sourcePath)) {
              ...
              componentNames.forEach((name) => {
                ...
                this.sources[name].push({ path: resource, context: context, priority: priority });
              });
            }
          }
        });
